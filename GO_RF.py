...
def objective(trial):
    params = {
        ...
        'max_samples': trial.suggest_float('max_samples', 0.7, 0.95) if trial.params['bootstrap'] else None,
    ...
    for train_idx, val_idx in kf.split(X_train):
        ...
        val_scores.append(np.sqrt(mean_squared_error(y_train.iloc[val_idx], preds)))

        if len(val_scores) >= 3 and np.mean(val_scores[-3:]) > np.mean(val_scores[:-3]):
            break
...
test_results.to_csv('rf_predictions.csv', index=False)
